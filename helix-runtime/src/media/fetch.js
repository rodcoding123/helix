...
export async function fetchRemoteMedia(options) {
    ...
    try {
        ...
        if (isPrivateIpAddress(hostname)) {
            throw new MediaFetchError("fetch_failed", `Blocked: private/internal IP address in media URL: ${url}`);
        }
        if (isBlockedHostname(hostname)) {
            throw new MediaFetchError("fetch_failed", `Blocked: internal hostname in media URL: ${url}`);
        }
    }
    catch (err) {
