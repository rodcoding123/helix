...
async function loadUsageWithProgress(runtime) {
    try {
        return await withProgress({ label: "Fetching usage snapshot…", indeterminate: true, enabled: true }, async () => await loadProviderUsageSummary());
    }
    catch (err) {
...
export async function channelsListCommand(opts, runtime = defaultRuntime) {
