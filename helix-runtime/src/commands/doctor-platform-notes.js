...
export async function noteMacLaunchctlGatewayEnvOverrides(cfg, deps) {
    ...
    if (deprecatedLaunchctlEntries.length > 0) {
        const lines = [
            ...
            ...deprecatedLaunchctlEntries.map(([key]) => `- \`${key}\` is set; use \`OPENCLAW_${key.slice(key.indexOf("_") + 1)}\` instead.`),
        ];
...
export function noteDeprecatedLegacyEnvVars(env = process.env, deps) {
    const entries = Object.entries(env)
        .filter(([key, value]) => (key.startsWith("MOLTBOT_") || key.startsWith("CLAWDBOT_")) && value?.trim())
        .map(([key]) => key);
    if (entries.length === 0) {
        return;
    }
...
//# sourceMappingURL=doctor-platform-notes.js.map