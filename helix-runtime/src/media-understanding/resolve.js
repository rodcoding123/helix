...
import { DEFAULT_MAX_BYTES, DEFAULT_MAX_CHARS_BY_CAPABILITY, DEFAULT_MEDIA_CONCURRENCY, DEFAULT_PROMPT, } from "./defaults.js";
import { normalizeMediaProviderId } from "./providers/index.js";
import { normalizeMediaUnderstandingChatType, resolveMediaUnderstandingScope } from "./scope.js";
export function resolveTimeoutMs(seconds, fallbackSeconds) {
...
export function resolvePrompt(capability, prompt, maxChars) {
...
export function resolveCapabilityConfig(cfg, capability) {
...
export function resolveModelEntries(params) {
    ...
    return entries
        ...
        if (!caps || caps.length === 0) {
            if (source === "shared") {
                if (shouldLogVerbose()) {
                    logVerbose(`Skipping shared media model without capabilities: ${entry.provider ?? entry.command ?? "unknown"}`);
                }
                return false;
            }
            return true;
