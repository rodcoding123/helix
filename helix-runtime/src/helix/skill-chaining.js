 ...
 */
export function validateCompositeSkill(skill) {
    const errors = [];
    if (!skill.id)
        ...
        for (let i = 0; i < skill.steps.length; i++) {
            ...
            if (step.inputMapping) {
                for (const [param, path] of Object.entries(step.inputMapping)) {
                    try {
                        ...
                        errors.push(`Step ${step.stepId}: Invalid JSONPath in inputMapping for param '${param}': ${path}`);
                    }
                }
            }
            ...
            if (step.outputMapping) {
                try {
                    ...
                    errors.push(`Step ${step.stepId}: Invalid JSONPath in outputMapping: ${step.outputMapping}`);
                }
            }
        }
    }
 ...
 */
export async function executeCompositeSkill(skill, input = {}, toolExecutor) {
...
function resolveInputMapping(inputMapping, context) {
    ...
    for (const [paramName, jsonPath] of Object.entries(inputMapping)) {
        try {
            ...
            throw new Error(`Failed to resolve input mapping for '${paramName}' with path '${jsonPath}': ${error}`);
        }
    }
...
async function executeTool(toolName, params) {
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            switch (toolName) {
                case 'double':
                    resolve({ result: params.x * 2 });
                    break;
                case 'add10':
                    resolve({ result: params.x + 10 });
                    break;
                case 'stringify':
...
//# sourceMappingURL=skill-chaining.js.map