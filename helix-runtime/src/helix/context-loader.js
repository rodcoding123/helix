 ...
 */
export async function loadHelixContextFiles(workspaceDir) {
    const results = [];
 ...
 */
export async function loadHelixContextFilesDetailed(workspaceDir) {
    const results = [];
    const axisDir = path.join(workspaceDir, "axis");
    for (const [layerNum, layer] of Object.entries(HELIX_LAYER_FILES)) {
        for (const relativePath of layer.files) {
 ...
 */
export async function getHelixContextStatus(workspaceDir) {
    const status = [];
    const axisDir = path.join(workspaceDir, "axis");
    for (const [layerNum, layer] of Object.entries(HELIX_LAYER_FILES)) {
        for (const relativePath of layer.files) {
 ...
 */
export async function ensureHelixDirectoryStructure(workspaceDir) {
    const directories = [
        "axis/psychology",
        "axis/identity",
        "axis/transformation",
 ...
 */
export async function isHelixConfigured(workspaceDir) {
    const axisDir = path.join(workspaceDir, "axis");
    try {
