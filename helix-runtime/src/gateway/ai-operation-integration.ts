...
function getRouter(): AIOperationRouter {
  if (!router) {
...
function getApprovalGate(): ApprovalGate {
  if (!approvalGate) {
...
export async function executeWithRouting<T>(
  ...
  try {
    ...
    if (context.requiresApproval()) {
      ...
      const approval = await approvalGateInstance.requestApproval(
        context.operationId,
 ...
 */
async function logOperationCost(context: OperationContext): Promise<void> {
...
): Promise<{ estimatedCostUsd: number; model: string }> {
  const routerInstance = getRouter();
