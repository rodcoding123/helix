...
export const calendarHandlers = {
    ...
    'calendar.add_event': async ({ params, respond, context }) => {
        ...
        try {
            const result = await context.db.query(`INSERT INTO calendar_events
    ...
    'calendar.get_events': async ({ params, respond, context }) => {
        const { user_id, start_time, end_time, account_id, limit = 50, offset = 0 } = params;
        try {
            const query = `
    ...
    'calendar.search_events': async ({ params, respond, context }) => {
        const { user_id, query, start_time, end_time, account_id, limit = 50, offset = 0 } = params;
        try {
            const queryStr = `
        SELECT * FROM calendar_events
        WHERE user_id = $1
    ...
    'calendar.get_event': async ({ params, respond, context }) => {
        ...
        try {
            const eventResult = await context.db.query('SELECT * FROM calendar_events WHERE id = $1', [event_id]);
            if (eventResult.rows.length === 0) {
                respond(false, { error: 'Event not found' });
                return;
            }
            const attendeesResult = await context.db.query('SELECT * FROM calendar_attendees WHERE event_id = $1 ORDER BY created_at ASC', [event_id]);
            respond(true, {
    ...
    'calendar.delete_event': async ({ params, respond, context }) => {
        ...
        try {
            ...
            const result = await context.db.query('DELETE FROM calendar_events WHERE id = $1 RETURNING id', [event_id]);
    ...
    'calendar.create_recurring': async ({ params, respond, context }) => {
        const { user_id, event, rrule } = params;
        try {
            const result = await context.db.query(`INSERT INTO calendar_events
    ...
    'calendar.update_attendees': async ({ params, respond, context }) => {
        const { event_id, attendees } = params;
        try {
            ...
            for (const attendee of attendees) {
                const result = await context.db.query(`INSERT INTO calendar_attendees (event_id, email, name, status)
          VALUES ($1, $2, $3, $4)
          RETURNING *`, [event_id, attendee.email, attendee.name || null, attendee.status || 'pending']);
    ...
    'calendar.sync_calendar': async ({ params, respond, context }) => {
        ...
        try {
            ...
            const syncResult = await context.db.query(`INSERT INTO calendar_sync_log (account_id, started_at, status)
        VALUES ($1, NOW(), $2)
        RETURNING *`, [account_id, 'running']);
            ...
            setImmediate(async () => {
                try {
                    ...
                    await context.db.query(`UPDATE calendar_sync_log SET completed_at = NOW(), status = $1, events_synced = $2 WHERE id = $3`, ['completed', mockEventsCount, syncId]);
                    await context.discord.send('commands', {
                    ...
                    await context.db.query(`UPDATE calendar_sync_log SET completed_at = NOW(), status = $1, error_message = $2 WHERE id = $3`, ['failed', error.message, syncId]);
                }
            });
    ...
    'calendar.get_sync_status': async ({ params, respond, context }) => {
        ...
        try {
            const result = await context.db.query('SELECT * FROM calendar_sync_log WHERE id = $1', [sync_id]);
            if (result.rows.length === 0) {
    ...
    'calendar.get_calendar_view': async ({ params, respond, context }) => {
        const { user_id, start_time, end_time, view_type, account_id } = params;
        try {
            const query = `
        SELECT * FROM calendar_events
        WHERE user_id = $1
