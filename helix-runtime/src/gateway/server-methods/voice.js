 ...
 * }
 */
async function handleUploadVoiceMemo(params, context, client) {
    ...
    try {
        const db = context.db;
        // Store voice memo
        await db.query(`INSERT INTO voice_memos
       (id, user_id, audio_url, duration_ms, file_size_bytes, title, transcription_status)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending')`, [memoId, userId, audioUrl, durationMs, fileSizeBytes || null, title || null]);
        // Queue for transcription
        await db.query(`INSERT INTO voice_processing_queue
 ...
 * }
 */
async function handleSearchTranscripts(params, context, client) {
    const userId = client?.connect?.userId;
    if (!userId) {
        return {
            success: false,
    ...
    try {
        ...
        const result = await db.query(`SELECT id, text, speaker, source, created_at
       FROM voice_transcripts
       WHERE user_id = $1
       AND to_tsvector('english', text) @@ plainto_tsquery('english', $2)
       ORDER BY created_at DESC
       LIMIT $3 OFFSET $4`, [userId, query, limit, offset]);
        const countResult = await db.query(`SELECT COUNT(*) as total
       FROM voice_transcripts
       WHERE user_id = $1
       AND to_tsvector('english', text) @@ plainto_tsquery('english', $2)`, [userId, query]);
 ...
 * }
 */
async function handleCreateVoiceCommand(params, context, client) {
    const userId = client?.connect?.userId;
    if (!userId) {
        return {
            success: false,
    ...
    try {
        ...
        if (commandId) {
            // Update existing
            await db.query(`UPDATE voice_commands
         SET trigger_phrase = $1, tool_id = $2, action_config = $3, updated_at = NOW()
         WHERE id = $4 AND user_id = $5`, [triggerPhrase, toolId, JSON.stringify(actionConfig || {}), id, userId]);
        }
        else {
            // Insert new
            await db.query(`INSERT INTO voice_commands
         (id, user_id, trigger_phrase, tool_id, command_type, action_config, is_enabled)
         VALUES ($1, $2, $3, $4, 'tool_execution', $5, true)`, [id, userId, triggerPhrase, toolId, JSON.stringify(actionConfig || {})]);
        }
 ...
 * }
 */
async function handleListVoiceCommands(context, client) {
    const userId = client?.connect?.userId;
    if (!userId) {
        return {
            success: false,
    ...
    try {
        const db = context.db;
        const result = await db.query(`SELECT id, trigger_phrase, tool_id, is_enabled, usage_count
       FROM voice_commands
       WHERE user_id = $1
       ORDER BY trigger_phrase ASC`, [userId]);
 ...
 * }
 */
async function handleUpdateVoiceSettings(params, context, client) {
    const userId = client?.connect?.userId;
    if (!userId) {
        return {
            success: false,
    ...
    try {
        ...
        await db.query(`UPDATE voice_settings
       SET ${updateFields.join(', ')}
       WHERE user_id = $1`, updateValues);
 ...
 * }
 */
async function handleGetVoiceSession(params, context, client) {
    const userId = client?.connect?.userId;
    if (!userId) {
        return {
            success: false,
    ...
    try {
        const db = context.db;
        const result = await db.query(`SELECT id, status, input_model, output_model, duration_seconds, start_time
       FROM voice_sessions
       WHERE session_key = $1 AND user_id = $2`, [sessionKey, userId]);
        if (!result.rows || result.rows.length === 0) {
            // Create new session
            const sessionId = crypto.randomUUID();
            await db.query(`INSERT INTO voice_sessions
         (id, user_id, session_key, status, input_model, output_model)
         VALUES ($1, $2, $3, 'active', 'deepgram', 'elevenlabs')`, [sessionId, userId, sessionKey]);
            return {
                success: true,
                sessionId,
                status: 'active',
