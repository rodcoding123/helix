...
const emailConversations = new Map<string, EmailConversation>();
const emailMessages = new Map<string, EmailMessage>();
const emailAttachments = new Map<string, EmailAttachment>();
...
function getAccountsByUser(userId: string): EmailAccount[] {
  return Array.from(emailAccounts.values()).filter(
    (account) => account.userId === userId && account.isActive,
  );
}

function getConversationsByAccount(
  accountId: string,
  options: {
    limit?: number;
    offset?: number;
  ...
  let conversations = Array.from(emailConversations.values()).filter(
    (conv) => conv.accountId === accountId,
  );
  ...
  if (labels && labels.length > 0) {
    conversations = conversations.filter((conv) =>
      labels.some((label) => conv.labels.includes(label)),
    );
  }
...
function searchConversations(
  ...
  if (from) {
    conversations = conversations.filter((conv) =>
      conv.participants.some((p) => p.email.toLowerCase() === from.toLowerCase()),
    );
  }
...
function getMessagesByConversation(conversationId: string): EmailMessage[] {
...
export const emailHandlers: GatewayRequestHandlers = {
  ...
  "email.add_account": async ({ params, respond }) => {
    ...
    if (!validation.valid) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, `Missing required parameters: ${validation.missing.join(", ")}`),
      );
      return;
    }

    const { userId, email, provider, authType, oauthToken, imapConfig } = params as {
      userId: string;
      email: string;
      provider: "gmail" | "outlook" | "yahoo" | "custom_imap";
      authType: "oauth" | "keyring";
    ...
    if (authType === "oauth" && !oauthToken) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "OAuth token required for OAuth authentication"),
      );
      return;
    }
    ...
    if (authType === "keyring" && !imapConfig) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "IMAP config required for keyring authentication"),
      );
      return;
    }

    // Check for duplicate account
    const existingAccount = Array.from(emailAccounts.values()).find(
      (acc) => acc.userId === userId && acc.email === email && acc.isActive,
    );
    ...
    if (existingAccount) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, `Account ${email} already exists`),
      );
      return;
    }
  ...
  "email.get_accounts": async ({ params, respond }) => {
    ...
    if (!userId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "userId is required"),
      );
      return;
    }
  ...
  "email.remove_account": async ({ params, respond }) => {
    const { accountId, userId } = params as { accountId: string; userId: string };
    ...
    if (!account || account.userId !== userId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "Account not found or unauthorized"),
      );
      return;
    }
  ...
  "email.sync_inbox": async ({ params, respond }) => {
    const { accountId, syncType = "incremental", daysToSync = 7 } = params as {
      accountId: string;
      syncType?: "incremental" | "full";
      daysToSync?: number;
    };
    ...
    if (!accountId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "accountId is required"),
      );
      return;
    }
  ...
  "email.get_conversations": async ({ params, respond }) => {
    const { accountId, limit = 50, offset = 0, includeRead = true, labels } = params as {
      accountId: string;
      limit?: number;
      offset?: number;
      includeRead?: boolean;
  ...
  "email.search_conversations": async ({ params, respond }) => {
    const { accountId, query, from, to, after, before, limit = 50 } = params as {
      accountId: string;
      query: string;
      from?: string;
      to?: string;
  ...
  "email.get_conversation": async ({ params, respond }) => {
    ...
    if (!conversationId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "conversationId is required"),
      );
      return;
    }
    ...
    if (!conversation) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "Conversation not found"),
      );
      return;
    }
  ...
  "email.send_message": async ({ params, respond }) => {
    const { accountId, to, cc, bcc, subject, bodyPlain, bodyHtml, inReplyTo } = params as {
      accountId: string;
      to: string[];
      cc?: string[];
      bcc?: string[];
  ...
  "email.mark_read": async ({ params, respond }) => {
    const { conversationId, isRead } = params as { conversationId: string; isRead: boolean };
    ...
    if (typeof isRead !== "boolean") {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "isRead must be a boolean"),
      );
      return;
    }
  ...
  "email.star_conversation": async ({ params, respond }) => {
    const { conversationId, isStarred } = params as { conversationId: string; isStarred: boolean };
    ...
    if (typeof isStarred !== "boolean") {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "isStarred must be a boolean"),
      );
      return;
    }
  ...
  "email.get_attachment": async ({ params, respond }) => {
    ...
    if (!attachmentId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "attachmentId is required"),
      );
      return;
    }
    ...
    if (!attachment) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "Attachment not found"),
      );
      return;
    }
...
export function __seedEmailAccountForTest(account: Partial<EmailAccount> & { id: string; userId: string; email: string }): EmailAccount {
  const now = Date.now();
  const fullAccount: EmailAccount = {
    provider: "gmail",
    authType: "oauth",
    isActive: true,
    syncState: {
...
export function __seedEmailConversationForTest(conversation: Partial<EmailConversation> & { id: string; accountId: string; userId: string }): EmailConversation {
  const now = Date.now();
  const fullConversation: EmailConversation = {
    threadId: randomUUID(),
...
export function __seedEmailMessageForTest(message: Partial<EmailMessage> & { id: string; conversationId: string; accountId: string }): EmailMessage {
  const now = Date.now();
...
export function __seedEmailAttachmentForTest(attachment: Partial<EmailAttachment> & { id: string; messageId: string }): EmailAttachment {
  const now = Date.now();
  const fullAttachment: EmailAttachment = {
    filename: "test.pdf",
