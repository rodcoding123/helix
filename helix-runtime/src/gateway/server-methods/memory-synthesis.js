...
export const memorySynthesisHandlers = {
    /**
    ...
    'memory.synthesize': async ({ params, respond, context, client }) => {
        ...
        try {
            ...
            const { synthesisType, conversations } = params;
            ...
            const conversationsText = conversations
                ...
                .join('\n\n');
            const promptTemplate = SYNTHESIS_PROMPTS[synthesisType];
            ...
            if (routingDecision.requiresApproval) {
                await approvalGate.requestApproval('memory_synthesis', 'Memory Synthesis', routingDecision.estimatedCostUsd || 0, `Synthesis Type: ${synthesisType} | Conversations: ${conversations.length}`, '');
            }
 ...
 */
function getModelClientForOperation(_model) {
