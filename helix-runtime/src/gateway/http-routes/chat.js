 ...
 */
async function handleChatHistory(req, res, context) {
    try {
 ...
 * }
 */
async function handleChatMessage(req, res, context) {
    ...
    try {
        ...
        const estimatedInputTokens = Math.ceil((JSON.stringify(messagesForAPI).length + message.length) / 4);
        ...
        if (routingDecision.requiresApproval) {
            const approval = await approvalGate.requestApproval('chat_message', 'Chat Message', routingDecision.estimatedCostUsd || 0, `User: ${userId} | Message length: ${message.length} chars | Model: ${routingDecision.model}`, userId || '');
        ...
        const response = await modelToUse.messages.create({
            ...
            max_tokens: 1024,
            system: "You are Helix, a helpful and thoughtful AI assistant. You remember what users share and reference it in future conversations. Be warm, authentic, and genuinely interested in helping.",
        ...
        try {
            const operationLatency = Date.now() - operationStartTime;
            await costTracker.logOperation(req.headers.authorization?.toString().slice(7) || 'unknown', {
                operation_type: 'chat_message',
                operation_id: 'chat_message',
                model_used: 'unknown',
                cost_usd: 0,
 ...
 */
function getModelClientForOperation(model) {
 ...
 */
function getModelIdForRoute(model) {
    const modelIds = {
 ...
 */
export async function handleChatHttpRequest(req, res, context) {
