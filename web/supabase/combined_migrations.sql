...
-- #####################################################
-- MIGRATION 006: UPDATE TIER NAMES + SESSIONS
-- #####################################################
...
ALTER TABLE subscriptions
  ...
  USING CASE tier::text
    ...
    WHEN 'Phantom' THEN 'phantom'::subscription_tier_new
    WHEN 'Overseer' THEN 'overseer'::subscription_tier_new
    WHEN 'Architect' THEN 'architect'::subscription_tier_new
    ELSE 'awaken'::subscription_tier_new
  END;

ALTER TABLE subscriptions ALTER COLUMN tier SET DEFAULT 'awaken';
...
CREATE POLICY sessions_user_policy ON sessions FOR ALL USING (user_id = auth.uid());
CREATE POLICY session_messages_user_policy ON session_messages
  FOR ALL USING (EXISTS (SELECT 1 FROM sessions WHERE sessions.id = session_messages.session_id AND sessions.user_id = auth.uid()));
...
CREATE POLICY session_transfers_user_policy ON session_transfers FOR ALL USING (user_id = auth.uid());
...
CREATE POLICY session_sync_state_policy ON session_sync_state
  FOR ALL USING (EXISTS (SELECT 1 FROM sessions WHERE sessions.id = session_sync_state.session_id AND sessions.user_id = auth.uid()));
...
CREATE POLICY session_conflicts_policy ON session_conflicts
  FOR ALL USING (EXISTS (SELECT 1 FROM sessions WHERE sessions.id = session_conflicts.session_id AND sessions.user_id = auth.uid()));
...
CREATE TRIGGER on_message_update_session_activity
  AFTER INSERT ON session_messages FOR EACH ROW EXECUTE FUNCTION update_session_last_activity();
...
--   - record_session_transfer()
--
-- Total triggers created: 4
--   - subscriptions_updated_at
--   - on_user_created
--   - on_heartbeat_update_instance
--   - on_message_update_session_activity
--
-- RLS enabled on all tables with appropriate policies
-- Realtime enabled for: session_messages, session_transfers, session_sync_state, session_conflicts
-- =====================================================
